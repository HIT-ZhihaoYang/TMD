...
class MultiScale_GraphConv(nn.Module):
    def __init__(self,
        ...
        self.mlp = MLP(in_channels * (num_scales + 1), [out_channels], dropout=dropout, activation=activation)
    ...
    def forward(self, x, joint_graph):
        ...
        #fuse A and joint_graph
        # A = torch.einsum('vu,uu->vu', A, joint_graph)
        A = torch.cat((A, joint_graph), dim=0)

        support = torch.einsum('vu,nctu->nctv', A, x)
        # support = support.view(N, C, T, self.num_scales, V)
        support = support.view(N, C, T, self.num_scales + 1, V)
        support = support.permute(0,3,1,2,4).contiguous().view(N, (self.num_scales+1)*C, T, V)
...
class MLP(nn.Module):
    def __init__(self, in_channels, out_channels, activation='relu', dropout=0):
        ...
        #     if dropout > 0.001:
        #         self.layers.append(nn.Dropout(p=dropout))
        #     self.layers.append(nn.Conv2d(channels[1], channels[1], kernel_size=1))
        #     self.layers.append(nn.BatchNorm2d(channels[1]))
        #     self.layers.append(activation_factory(activation))
