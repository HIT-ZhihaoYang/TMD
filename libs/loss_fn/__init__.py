...
class KLLoss(nn.Module):
    ...
    def __init__(self, error_metric=nn.KLDivLoss(size_average=True, reduce=True)):
        ...
        self.error_metric = error_metric # 初始化 KL 散度损失函数，size_average 和 reduce 参数控制是否对损失进行平均和是否进行降维

    def forward(self, prediction, label):
        epsilon = 1e-8
        batch_size = prediction.shape[0] # 获取批次大小
        probs1 = F.log_softmax(prediction, 1) + epsilon # 对模型的预测结果和标签进行 softmax 操作再log
        probs2 = F.softmax(label * 10, 1) + epsilon # 在标签上应用温度调节
